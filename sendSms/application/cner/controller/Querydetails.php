...
class Querydetails extends Common{
    ...
    public function shup(){
        ...
        $biz=input('post.biz/a');
        for($i=0;$i<count($biz);$i++){
            $res=explode(',',$biz[$i]);
            if($res[4]!=3){
                $senddate=date('Y-m-d',$res[1]);
                $sendsdate=str_replace("-","",$senddate);
                $query = $this->queryDetails($res[0],$sendsdate,10,1,$res[2]);
                if(empty($slog['SmsSendDetailDTOs']['SmsSendDetailDTO'])){
                    continue;
                }else{
                    $slog=json_decode(json_encode($query),TRUE);
                    $c=count($slog['SmsSendDetailDTOs']['SmsSendDetailDTO']);
                    for($j=0;$j<$c;$j++){
                        date_default_timezone_set('PRC');
                        if(strtotime($slog['SmsSendDetailDTOs']['SmsSendDetailDTO'][$j]['SendDate'])==$res[1]){
                            $slog=$slog['SmsSendDetailDTOs']['SmsSendDetailDTO'][$i];
                        }
                    }
                }
                if($errmess[$slog['ErrCode']]!=''){$slog['ErrCode']=$errmess[$slog['ErrCode']];};
                date_default_timezone_set('PRC');
                if(isset($slog['ReceiveDate'])){
                    db('searchlog')->where('id',$res[3])->update(['SendDate'=>strtotime($slog['SendDate']),'ReceiveDate'=>strtotime($slog['ReceiveDate']),'ErrCode'=>$slog['ErrCode'],'SendStatus'=>$slog['SendStatus']]);
                }
            }
        }
        return show(1,'更新成功');
